...
function createTodoTool(api: ExtensionAPI): ToolDefinition<typeof todoWriteSchema, TodoToolDetails> {
	...
	const tool: ToolDefinition<typeof todoWriteSchema, TodoToolDetails> = {
		name: "todos",
		label: "Todo",
		description: "Manage a structured todo list with phases. Tasks auto-normalize to single in_progress. State persists across sessions via ./.pi/agent/todos.json",
		promptSnippet: "todos({ add_phase:{ name:'Phase', tasks:[{content:'Task'}] }, add_task:{ phase:'phase-1', content:'Task' }, update:{ id:'task-1', status:'completed' }, list:{} })",
		promptGuidelines: [
